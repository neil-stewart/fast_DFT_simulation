...
//
// DFT_parallel.c is free software: you can redistribute it and/or modify it under
// the terms of the GNU General Public License as published by the Free
// Software Foundation, either version 3 of the License, or (at your option)
// any later version.
//
// DFT_parallel.c is distributed in the hope that it will be useful, but WITHOUT ANY
...
// Version 1.0.1
//
// Change for loop accumulating local_counter to be critical not atomic to fix 
// problem with incorrect accumulation of counter[1]. Counter[0] (and so the 
// proportion of A responses) was not broken.
//
...
void
simulate_DFT_parallel(double *Cin, double *Min, double *Win, double *Sin,
...
#pragma omp parallel private(P, SP, diff, epsilon, local_counter, step_counter)
	{
		P = gsl_vector_alloc(n);
		SP = gsl_vector_alloc(n);
		diff = gsl_vector_alloc(n);
		epsilon = gsl_vector_alloc(n);
		local_counter = calloc(n, sizeof(int));
#pragma omp for
		for (i = 0; i < *iter; i++) {
			...
			while (gsl_vector_isnonneg(diff) && step_counter < 10000);
			//fprintf(stderr, "step counter=%d\n", step_counter);
			if(step_counter < 10000)
				local_counter[gsl_vector_min_index(diff)]++;	// Get the index of the element in P which exceeds threshold
			else
		...
		// Add local counters from each thread into the main counter
#pragma omp critical
		for (j = 0; j < n; j++) {
			counter[j] += local_counter[j];
		}
		free(local_counter);
	}
...
/* Useful for debugging, not used by the R call */
{
	...
	simulate_DFT_parallel(C, M, W, S, T, Pstart, &sigma, &n, &d, &iter, &seed,
	  counter);
